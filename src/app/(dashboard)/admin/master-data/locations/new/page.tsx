...
export default async function NewLocationPage() { const { user } = await requireAuthenticatedUser(); if (!canMutateLocations(user)) notFound(); return <><PageHeader title="New Location" description="Create an official routing Location for the reusable catalog."><Link className="rounded-md border border-border px-4 py-2 text-sm font-medium hover:bg-muted" href="/admin/master-data/locations">Back to Locations</Link></PageHeader><div className="mt-6"><CreateLocationForm /></div></>; }
