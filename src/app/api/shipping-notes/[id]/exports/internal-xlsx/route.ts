...
function toExportError(error: unknown): ExportError {
  ...
  if (
    ...
    return new ExportError(
      ...
      409,
      "Shipping note is not eligible for internal XLSX export.",
    );
  }
...
export async function POST(
  ...
  const fileName = buildInternalXlsxFileName(exportData, generatedAt);
  let exportRecordId: string | null = null;

  try {
    const exportRecord = await createPendingInternalXlsxExportRecord({
    ...
    return new Response(generated.buffer, {
      status: 200,
      headers: {
        ...noStoreHeaders(),
        "Content-Type": INTERNAL_XLSX_MIME_TYPE,
        "Content-Disposition": buildContentDisposition(generated.fileName),
        "Content-Length": generated.buffer.byteLength.toString(),
        "X-Template-Version": generated.templateVersion,
      },
    });
  } catch (error) {
    const exportError = toExportError(error);
    ...
    if (exportRecordId) {
      try {
        await markInternalXlsxExportFailed({
