...
vi.mock("@/features/shipping-notes/export/pdf/generator", () => ({
  generateInternalShippingNotePdf: vi.fn(),
}));

vi.mock("@/features/shipping-notes/export/filename", () => ({
  buildInternalPdfFileName: vi.fn(),
}));

vi.mock("@/features/shipping-notes/export/mutations", () => ({
  createPendingInternalPdfExportRecord: vi.fn(),
  markInternalPdfExportFailed: vi.fn(),
  markInternalPdfExportGenerated: vi.fn(),
}));
...
function makeRequest(origin: string): NextRequest {
  return new NextRequest(
    "http://railway-internal:8080/api/shipping-notes/note-1/exports/internal-pdf",
    {
      method: "POST",
      headers: {
        origin,
...
describe("internal PDF export origin guard", () => {
  beforeEach(() => {
